...
def format_pile(stones_in_pile: int) -> str:
  ...
  """
  stones_str = "\n"
  row_max = int(math.ceil(math.sqrt(stones_in_pile)))
  full_rows = stones_in_pile//row_max
  for row in range(full_rows):
      row = '*'*row_max
      stones_str = stones_str+row+"\n"

  if int(stones_in_pile%row_max) > 0:
      left_over = int(stones_in_pile%row_max)
      last_row = '*'*left_over
      stones_str = stones_str+last_row+"\n"

  if stones_in_pile > 1:
      stones_str += f"There are {str(stones_in_pile)} stones in the pile."
  else:
...
def get_ai_guess(stones_in_pile:int, valid_guesses:list) -> int:
  """Picks the perfect number of stones to eventually leave the other player with 1 remaining stone.
     If possible, a perfect strategy for 16 Stones is employed. Otherwise, the AI picks 1 to maximize 
     room for the other the player to make a mistake.
      ...
      1
  """
  stones_taken = (stones_in_pile - 1) % 5
  if stones_taken == 0:
      stones_taken = 1
...
def play_game(num_players:int)->int:
  ...
  while stones_in_pile > 1:
    ...
    #The next player picks some stones
