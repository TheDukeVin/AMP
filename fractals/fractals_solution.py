...
def circleTree(center_x, center_y, radius,  color):
    canvas.create_oval(center_x-radius, center_y-radius, center_x+radius, center_y+radius, outline=color)#top left, botom right

    if radius > 1:
      circleTree(center_x-radius, center_y, radius*0.5, color)
      circleTree(center_x+radius, center_y, radius*0.5,  color)

def circleTiles(center_x, center_y, radius, color):
    canvas.create_oval(center_x-radius, center_y-radius, center_x+radius, center_y+radius, outline=color)#top left, botom right

    if radius > 3:
      circleTiles(center_x-radius, center_y, radius*0.5, color)
      circleTiles(center_x+radius, center_y, radius*0.5, color)
      circleTiles(center_x, center_y+radius, radius*0.5, color)
      circleTiles(center_x, center_y-radius, radius*0.5, color)
...
if __name__ == "__main__":
    ...
    while play:
        ...
        match choice:
            ...
            case '1':
              circleTree(canvas_width/2, canvas_height/2, canvas_width/2, 'black')
            case '2':
              circleTiles(canvas_width/2, canvas_height/2, canvas_width/2, 'black')
            case _:
              play = False
