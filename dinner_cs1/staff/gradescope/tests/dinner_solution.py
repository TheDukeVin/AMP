...
def generate_all_subsets_solution(friends: list[str])->list[str]:
...
def filter_bad_invites_solution(all_subsets:list, friends:dict)-> list:
...
def invite_to_dinner_all_solution(friends: dict):
    '''Finds the invite combo with the maximum number of guests
       TODO: Needs comments
    '''
    definite_invites, problem_friends = filter_no_dislikes(friends)
    all_subsets = generate_all_subsets_solution(problem_friends)
    good_subsets = filter_bad_invites_solution(all_subsets, problem_friends)
...
def invite_to_dinner_optimized_solution(friends:dict)->list:
...
def invite_to_dinner_with_ties(friends: dict):
    '''Finds ALL invite combos with the maximum number of guests

    Guarantees guests are listed in alphabetical order.
    '''
    definite_invites, problem_friends = filter_no_dislikes(friends)
    all_subsets = generate_all_subsets_solution(problem_friends)
    good_subsets = filter_bad_invites_solution(all_subsets, problem_friends)

    #find the subset(S) which maximize number of guests
    size_of_subsets = [len(subset) for subset in good_subsets]
    max_size = max(size_of_subsets)
    invite_lists = []
    for subset in good_subsets:
        if len(subset) == max_size:
            #recombine with definite invites
            invite_lists.append(sorted(subset + definite_invites))
...
if __name__ == "__main__":
    ...
    print(invite_to_dinner_optimized_solution(friends))
    print(invite_to_dinner_optimized_solution(friends))
    ...
    s=generate_all_subsets_solution(friends_2)
    print(s)
    print(filter_bad_invites_solution(s, friends_2))
    print(invite_to_dinner_optimized_solution(friends_4))
    print(invite_to_dinner_optimized_solution(friends_4))
