...
def generate_letters(fun_factor: int, distribution: str, explorer:AnagramExplorer_solution) -> list:
...
def play_game(time_limit: int, letters: list, explorer:AnagramExplorer_solution) -> list:
...
def calc_stats(guesses: list[tuple], letters: list, explorer) -> dict:
    ...
    for guess in guesses:
        if explorer.is_valid_anagram_pair(guess, letters):
            if(tuple(sorted(guess)) not in already_guessed):
                already_guessed.add(tuple(sorted(guess)))
                stats["valid"].append((guess[0], guess[1]))
                stats["score"] += len(guess[0])-2
                stats["guessed"].add(guess[0])
                stats["guessed"].add(guess[1])
            else:
                stats["invalid"].append((guess[0], guess[1]))
        else:
            stats["invalid"].append((guess[0], guess[1]))

    if len(guesses) != 0: stats["accuracy"] = int(len(stats["valid"])/len(guesses)*100)

    stats["not guessed"] = explorer.get_all_anagrams(letters)
    num_unique_words = len(stats["not guessed"])
    for word in stats["guessed"]:
      stats["not guessed"].remove(word)

    stats["skill"] = int(len(stats["guessed"])/num_unique_words *100)
...
if __name__ == "__main__":
  ...
  explorer = AnagramExplorer_solution(get_valid_word_list())
