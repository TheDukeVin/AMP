...
def get_sorted_hash_dict(corpus: list) -> dict:
    ...
    for word in corpus:
        word = word.lower()
        sorted_string_key = tuple(sorted(word))
        if sorted_string_key not in lookup_dict:
            lookup_dict[sorted_string_key] = [word]
        else:
            lookup_dict[sorted_string_key].append(word)
...
def get_most_anagrams(corpus:list)->list:
    ...
    most_words = 2
    most_anagrams = []
    for wordList in lookup_dict.values():
        if len(wordList) > most_words:
            most_anagrams = [sorted(wordList)[0]]
            most_words = len(wordList)
        elif len(wordList) == most_words:
            most_anagrams.append(sorted(wordList)[0])
...
def get_all_anagrams(corpus:list[str])->set:
    '''Creates a set of all unique words that could have been used to form an anagram pair.
        ...
        Examples
      ----------
      >>>et_all_anagrams(["abed","mouse", "bead", "baled", "abled", "rat", "blade"])
      {"abed",  "abled", "baled", "bead", "blade"}
    '''
    all_anagrams=set()
    
    lookup = get_prime_hash_dict(corpus)
    for anagram_list in lookup.values():
...
if __name__ == "__main__":
    ...
    print()
    all_anagrams = get_all_anagrams(words2)
    print(f"all anagrams in words2: {all_anagrams}")
