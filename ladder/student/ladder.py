...
def shortest_ladder(graph:dict[str, set[str]], start:str, target:str, word_list:list[str])->list[str]:
        ...
        for mask in get_letter_masks(last_rung):
            for neighbour in graph[mask]:
                if neighbour not in old_ladder:
                    new_ladder = list(old_ladder) #makes copy of old ladder to allow for other branches of the search tree
...
def all_shortest_ladders(graph:dict[str, set[str]], start:str, target:str, word_list:list[str])->set[tuple[str]]:
...
def all_ladders(graph:dict[str, set[str]], start:str, target:str, word_list:list[str], max_rungs:int)->set[tuple[str]]:
