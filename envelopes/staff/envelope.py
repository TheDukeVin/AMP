...
def to_binary_format(n):
    #Approach #1: Built-in function: format
    binary = format(n, 'b')
    return binary
...
def to_binary_divide(n):
    ...
    binary = ""
    if n == 0: return "0"  #Mistake: return 0
...
def to_binary_subtract(n):
    ...
    while power_of_2 >= 0:
        if n >= 2**power_of_2:
            binary = binary + "1"  #Mistake: binary = "1" + binary
            n=n-2**power_of_2
        else:
            binary = binary + "0"  #Mistake: binary = "0" + binary
...
if __name__ == '__main__':
    ...
    print( to_binary_bin(n) ) #Mistake: print( to_Binary_Format(n) )
    #Mistake:    print( to_binary_bitwise(n) )  ... incorrect indentation
